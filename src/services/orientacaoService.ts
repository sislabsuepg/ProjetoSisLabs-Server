...
export default class OrientacaoService {
  ...
  static async createOrientacao(
    ...
    try {
      ...
      await criarRegistro(
        idUsuario,
        `Criou orientação: ra=${aluno!.ra}; aluno=${aluno!.nome}; lab=${
          laboratorio!.nome
        }; prof=${professor!.nome}`
      );
  ...
  static async updateOrientacao(
    ...
    try {
      ...
      await criarRegistro(
        idUsuario,
        `Atualizou orientação: inicio=${orientacao.dataInicio?.toISOString()}; fim=${orientacao.dataFim?.toISOString()}`
      );
  ...
  static async deleteOrientacao(id: number, idUsuario?: number) {
    try {
      ...
      await criarRegistro(
        idUsuario,
        `Desativou orientação: aluno=${orientacao.getDataValue(
          "idAluno"
        )}; lab=${orientacao.getDataValue("idLaboratorio")}`
      );
