...
class RelatorioController {
    async gerarRelatorioAcademicoPorCurso(req: Request, res: Response) {
        ...
        try {
            ...
            if (!res.headersSent) {
                res.status(codes.INTERNAL_SERVER_ERROR).send('Erro ao gerar o relatório de Acadêmicos por Curso.');
            }
        }
    }
    ...
    async gerarRelatorioAcademico(req: Request, res: Response) {
        ...
        try {
            ...
            if (!res.headersSent) {
                res.status(codes.INTERNAL_SERVER_ERROR).send('Erro ao gerar o relatório de Acadêmico.');
            }
        }
    }
    ...
    async gerarRelatorioEmprestimo(req: Request, res: Response) {
        ...
        try {
            res.setHeader('Content-Type', 'application/pdf');
            res.setHeader('Content-Disposition', `attachment; filename=relatorio_emprestimos.pdf`);
            ...
            if (!res.headersSent) {
                res.status(codes.INTERNAL_SERVER_ERROR).send('Erro ao gerar o relatório de Empréstimos.');
            }
        }
    }
}
