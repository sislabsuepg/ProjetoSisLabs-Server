...
function desenharCabecalhoTabela(doc: PDFKit.PDFDocument, y: number) {
    ...
    doc
        .fontSize(10)
        .font('Helvetica-Bold')
        .rect(50, y, colWidths.reduce((a, b) => a + b), itemHeight)
...
export function addTabela(doc: PDFKit.PDFDocument, data: dadosRelatorio[], titulo: string) {
    ...
    for (const row of data) {
        ...
        const rowData = [ row.nome, row.ra, row.posseChave, row.dataHoraEntrada, row.dataHoraSaida ];
        let currentX = 55;

        rowData.forEach((cell, i) => {
            doc.fillColor(CORES.cinzaEscuro).text(cell.toString(), currentX, y + 8, { width: colWidths[i] - 10, align: 'left' });
            currentX += colWidths[i];
        });
