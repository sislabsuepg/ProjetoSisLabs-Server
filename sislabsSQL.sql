...
CREATE TABLE IF NOT EXISTS emprestimo ( id SERIAL PRIMARY KEY, "dataHoraEntrada" TIMESTAMPTZ NOT NULL, "dataHoraSaida" TIMESTAMPTZ, "posseChave" BOOLEAN NOT NULL DEFAULT FALSE, advertencia VARCHAR(255), "idAluno" INTEGER REFERENCES aluno(id) ON UPDATE CASCADE ON DELETE SET NULL, "idUsuarioEntrada" INTEGER REFERENCES usuario(id) ON UPDATE CASCADE ON DELETE SET NULL, "idUsuarioSaida" INTEGER REFERENCES usuario(id) ON UPDATE CASCADE ON DELETE SET NULL, "idLaboratorio" INTEGER REFERENCES laboratorio(id) ON UPDATE CASCADE ON DELETE SET NULL, CONSTRAINT emprestimo_chk_datas CHECK ("dataHoraSaida" IS NULL OR "dataHoraSaida" > "dataHoraEntrada") );
...
-- Seed permissão admin e usuário inicial
